...
let state = loadState();
let youtubeApiPromise = null, segmentPlayer = null, segmentGuard = null, playbackStartTimer = null, activeSegment = null;
...
function resetPlayer(){if(segmentGuard)clearInterval(segmentGuard);segmentGuard=null;if(playbackStartTimer)clearTimeout(playbackStartTimer);playbackStartTimer=null;if(segmentPlayer&&typeof segmentPlayer.destroy==="function")segmentPlayer.destroy();segmentPlayer=null;activeSegment=null;els.playerFrame.innerHTML="";}
...
function mountSegmentPlayer(entry,segment){if(!els.playerDialog.open||activeSegment?.id!==segment.id||!window.YT?.Player)return;if(playbackStartTimer)clearTimeout(playbackStartTimer);playbackStartTimer=null;const playerVars={autoplay:1,controls:1,rel:0,playsinline:1,start:segment.start};if(Number.isFinite(segment.end))playerVars.end=segment.end;if(location.origin.startsWith("http"))playerVars.origin=location.origin;segmentPlayer=new window.YT.Player("ytSegmentPlayer",{width:"100%",height:"100%",videoId:entry.videoId,playerVars,events:{onReady:(ev)=>{const videoOptions={videoId:entry.videoId,startSeconds:segment.start};if(Number.isFinite(segment.end))videoOptions.endSeconds=segment.end;ev.target.getIframe().tabIndex=-1;ev.target.loadVideoById(videoOptions);ev.target.playVideo();schedulePlaybackCheck(segment);keepPlayerDialogFocus();startSegmentGuard(segment);},onStateChange:(ev)=>{if(!window.YT||!activeSegment)return;keepPlayerDialogFocus();if(ev.data===window.YT.PlayerState.PLAYING){hidePlayerStatus();startSegmentGuard(segment);}if(ev.data===window.YT.PlayerState.ENDED)resetSegmentToStart(false);},onError:()=>showPlayerError()}});}
function schedulePlaybackCheck(segment){if(playbackStartTimer)clearTimeout(playbackStartTimer);playbackStartTimer=setTimeout(()=>{if(!segmentPlayer||activeSegment?.id!==segment.id||typeof segmentPlayer.getPlayerState!=="function")return;const playerState=segmentPlayer.getPlayerState();if(![window.YT.PlayerState.PLAYING,window.YT.PlayerState.BUFFERING].includes(playerState))showPlaybackPrompt();},1600);}
function showPlaybackPrompt(){const overlay=$("#playerStatusOverlay");if(!overlay)return;overlay.classList.remove("hidden");overlay.innerHTML='<button class="player-start-button" type="button" data-start-playback><span class="play-icon"></span><span>Start segment</span></button><small>Your browser prevented automatic playback.</small>';}
...
function startBlockedPlayback(){if(!segmentPlayer||!activeSegment)return;hidePlayerStatus();segmentPlayer.seekTo(activeSegment.start,true);segmentPlayer.playVideo();schedulePlaybackCheck(activeSegment);keepPlayerDialogFocus();}
