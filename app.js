...
function startBlockedPlayback(){if(!segmentPlayer||!activeSegment)return;if(playbackStartTimer)clearTimeout(playbackStartTimer);playbackNeedsSoundGesture=false;segmentPlayer.unMute();segmentPlayer.seekTo(activeSegment.start,true);segmentPlayer.playVideo();playbackStartTimer=setTimeout(()=>{if(!segmentPlayer||!activeSegment)return;if(typeof segmentPlayer.isMuted==="function"&&segmentPlayer.isMuted()){playbackNeedsSoundGesture=true;showPlaybackPrompt();}else hidePlayerStatus();},500);}
